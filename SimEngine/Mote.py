...
# to fix bug with timer that doesn't fire
# python runSimOneCPU.py --numRuns=1 --numCyclesPerRun=200 --seed=6 --numMotes=25 --gui --convergeFirst=1 --sf=resf --sixtopMessaging=1

# to fix bug with counters
# python runSim.py --sf=ellsf --numMotes=3 --seed=6 --convergeFirst=1 --gui --numRuns=1 --numCycles=1000
...
class Mote(object):
    ...
    def __init__(self, id):
        ...
        self.genReSF.seed(self.settings.seed + self.id)
        ...
        self.resfCellsTotal = 0
        self.resfCellsNoTx = 0
        self.resfCellsTx = 0
    ...
    def _app_action_receivePacket(self, srcIp, payload, timestamp):
        ...
        # update mote stats
        if self.settings.convergeFirst and payload[
            1] >= self.engine.asnInitExperiment and self.engine.asn <= self.engine.asnEndExperiment:
            self.pktReceived += 1
    ...
    def _app_action_enqueueData(self):
        ...
        if self.settings.convergeFirst and self.engine.asn >= self.engine.asnInitExperiment:
        ...
        #         (self.ReSF is None and self.numCellsToNeighbors.get(self.preferredParent, 0) != 0) or (
        #         self.ReSF is not None and self.preferredParent in self.ReSF.txTuples)):

        if self.preferredParent and \
                ((self.ReSF is None and self.numCellsToNeighbors.get(self.preferredParent, 0) != 0) or \
                (self.ReSF is not None and ((self.numCellsToNeighbors.get(self.preferredParent, 0) != 0) or self.preferredParent in self.ReSF.txTuples))):
            ...
            if self.ReSF is not None:
                ...
                newPacket = {
                    ...
                    'type': APP_TYPE_DATA,
                    'code': None,
                    'payload': [self.id, self.engine.getAsn(), 1, self.ReSF.tuple['uniqueId']],
                    # the payload is used for latency and number of hops calculation
                    'retriesLeft': TSCH_MAXTXRETRIES,
                    'srcIp': self,
                    'dstIp': self.dagRootAddress,
    ...
    def _tsch_action_enqueueEB(self):
        ...
        # if self.dagRoot \
        #     or (self.ReSF is None and self.preferredParent and self.numCellsToNeighbors.get(self.preferredParent, 0) != 0) \
        #     or (self.ReSF is not None and self.preferredParent):

        if self.dagRoot \
            or (self.preferredParent and self.numCellsToNeighbors.get(self.preferredParent, 0) != 0):
        # if self.dagRoot \
        #         or (self.ReSF is None and self.preferredParent) \
        #         or (self.ReSF is not None and self.preferredParent):
    ...
    def _rpl_action_enqueueDAO(self):
        ...
        if self.preferredParent and (
                (self.ReSF is None and self.numCellsToNeighbors.get(self.preferredParent, 0) != 0) or
                (self.ReSF is not None and self.preferredParent in self.ReSF.txTuples and len(self.ReSF.txTuples[self.preferredParent]) > 0)):
    ...
    def _rpl_action_receiveDIO(self, type, smac, payload):
        ...
        with self.dataLock:
            ...
            # if self.preferredParent is not None:
            #     self._log(INFO, "[rpl] My preferred parent is {0}", (self.preferredParent.id,))
            # else:
            #     self._log(INFO, "[rpl] Node {0} does not have a preferred parent yet.", (self.id,))
            # 
            ...
            # only in the case of:
            # * ReSF
            # * if you have a preferredParent
            # * you did not converge first
            if self.ReSF and self.settings.minCellsMSF == 0 and self.preferredParent and self.settings.convergeFirst and self.isConverged == False:
                self.isConverged = True
                if all(mote.isConverged == True for mote in
                       self.engine.motes) and self.engine.dedicatedCellConvergence == 99999999:
                    self._log(
                        INFO,
                    ...
                    # for m in self.engine.motes:
                    #     if m.id != 0:
                    #         print 'mote %d: prefparent %d' % (m.id, m.preferredParent.id)
                    # assert False
                    self.engine.dedicatedCellConvergence = self.engine.getAsn()
                    if self.ReSF is not None:  # if every mote converged, start sending ReSF messages
                        self.engine.ReSFEngine.converged()
                    else:  # only in the no ReSF case, we call this convergence
                        # experiment time in ASNs
                        simTime = self.settings.numCyclesPerRun * self.settings.slotframeLength
                        # offset until the end of the current cycle
                        offset = self.settings.slotframeLength - (self.engine.asn % self.settings.slotframeLength)
                    ...
                    self._log(
                        INFO,
                        "{0} motes converged with a dedicated cell in rpl housekeeping: {1}.",
                        (len(cvrgd), str(cvrgd))
                    )
            ...
            for (neighbor, neighborRank) in self.neighborRank.items():
                ...
                if (((self.settings.changeParent == 1 and (self.preferredParent == None or (self.engine.asn > self.engine.asnInitExperiment and self.preferredParent != None))) or \
                    ...
                    if skipNeighbor == True:
                        continue
                    if self.settings.dioFilter == 'children':
                        # nrRXs = neighbor.getRxCells(None, False)
                        chldrn = list(set(neighbor.childrenRPL))

                        # if len(nrRXs) > 0:
                        #     for (tsTmp, chTmp, nTmp) in nrRXs:
                        #         if nTmp.id not in chldrn:
                        #             chldrn.append(nTmp.id)
                        if neighbor.id == 0 or (len(chldrn) < self.settings.maxChildren and neighbor.id != 0):
                            # record this potential rank
                            potentialRanks[neighbor] = neighborRank + rankIncrease
                    elif self.settings.dioFilter == 'shortestDistance':
                        if self.closestNeighbor.id == neighbor.id:
                            potentialRanks[neighbor] = neighborRank + rankIncrease
                    else:
                        potentialRanks[neighbor] = neighborRank + rankIncrease
    ...
    def _msf_action_parent_change(self):
        ...
        with self.dataLock:
            ...
            if self.numCellsToNeighbors.get(self.preferredParent, 0) == 0:
                ...
                if self.ReSF is None or (self.ReSF is not None and self.settings.minCellsMSF > 0):
                    self._log(INFO,
                ...
                # make sure that this ReSF parent change is only called when there is an old parent,
                # meaning that it should not be called when there is no preferred parent yet,
                # only when there is a parent change
                if self.ReSF is not None and self.settings.resfChangeParent == 1 and self.oldPreferredParent is not None:
                    self._log(INFO,
                              "[resf] Triggering a ReSF parent change from parent {0} to parent {1}",
                              (self.oldPreferredParent.id, self.preferredParent.id))

                    if self.settings.resfChangeParentPolicy == 'delete' or self.settings.resfChangeParentPolicy == 'combined':
                        ...
                        lst_ts = self.ReSF._resf_get_uniqueids_timestamps(self.oldPreferredParent)

                    self.ReSF._resf_parent_change(self.oldPreferredParent)

                    if (self.settings.resfChangeParentPolicy == 'delete' or self.settings.resfChangeParentPolicy == 'combined') and \
                            (self.oldPreferredParent in self.ReSF.txTuples or self.oldPreferredParent in self.ReSF.txBlockedTuples):
                        self.ReSF._resf_send_delete(lst_ts, ReSF.RESF_THISPARENT, parent_id=self.oldPreferredParent.id)
                ...
                # armTimeout = True
            # elif self.numCellsToNeighbors.get(self.oldPreferredParent, 0) > 0:
            #     # the case where we just requested for cells to the new parent, but no reservations are complete yet
            #     armTimeoutRemoval = True
            #     self._log(INFO,
            #               "[msf] scheduling triggering 6P REMOVE of {0} cells, dir {1}, to mote {2}, 6P timeout {3} (parent change)",
            #               (self.settings.msfNumCellsToAddOrRemove, celloptions, self.oldPreferredParent.id, timeout,))
            ...
            #         priority=4,
            #     )
            # if armTimeoutRemoval:
            #     self.engine.scheduleIn(
            #         delay=400,
            #         cb=self._msf_action_parent_change,
            #         uniqueTag=(self.id, '_msf_action_parent_change_removal'),
            #         priority=4,
            #     )
            # if armTimeout:
    ...
    def _msf_get_sixtop_timeout(self, neighbor):
        ...
        for (ts, cell) in self.schedule.iteritems():
            if (cell['neighbor'] == neighbor and cell['dir'] == DIR_TX and not cell['resf']) or (
                    cell['dir'] == DIR_TXRX_SHARED and cell['neighbor'] == neighbor and not cell['resf']):
    ...
    def _msf_signal_cell_used(self, neighbor, cellOptions, direction=None, type=None):
        ...
        with self.dataLock:
            ...
            if cellOptions == DIR_TXRX_SHARED and neighbor == self.preferredParent and self.settings.resfNoMSF != 'no':
                self._log(INFO,
    ...
    def _msf_signal_cell_elapsed(self, neighbor, direction):
        ...
        with self.dataLock:
            ...
            if direction == DIR_TXRX_SHARED and neighbor == self.preferredParent and self.settings.resfNoMSF != 'no':
                self.numCellsElapsed += 1

                if self.numCellsElapsed == self.settings.msfMaxNumCells and self.settings.resfNoMSF != 'no':
                    self._log(INFO, '[msf] _msf_signal_cell_elapsed: numCellsElapsed = {0}, numCellsUsed = {1}',
                              (self.numCellsElapsed, self.numCellsUsed))

                    if (self.numCellsUsed > self.settings.msfLimNumCellsUsedHigh and \
                            ((self.ReSF is None) or \
                            ((self.ReSF is not None) and ((self.settings.resfNoMSF == 'no' and False) or \
                                                          (self.settings.resfNoMSF == 'onecell' and self.numCellsToNeighbors.get(self.preferredParent, 0) <= 1) or \
                                                            self.settings.resfNoMSF == 'unlimited')))):
    ...
    def _msf_action_bandwidth_increment(self):
        ...
        # # if we use ReSF, check the duration of this newly allocated cell
        self._sixtop_cell_reservation_request(self.preferredParent,
    ...
    def _msf_action_bandwidth_decrement(self):
        ...
        if self.numCellsToNeighbors.get(self.preferredParent, 0) > self.settings.minCellsMSF and self.settings.resfNoMSF != 'no':
            timeout = self._msf_get_sixtop_timeout(self.preferredParent)
            celloptions = DIR_TXRX_SHARED
            self._log(INFO,
    ...
    def _sixtop_timer_fired(self):
        ...
        for n in self.sixtopStates.keys():
            if 'tx' in self.sixtopStates[n] and 'timer' in self.sixtopStates[n]['tx'] and \
                ...
                if self.ReSF is not None:
                    txFound = False
                    # check if it was a ReSF add reservation, clear the blocked cells
                    for moteTmp in self.ReSF.txBlockedTuples.keys():
                        if moteTmp.id == n:
                            txFound = True
                            # here the ReSF reservation should first be re added
                            self.ReSF._resf_delayReservation(copy.deepcopy(self.ReSF.txBlockedTuples[moteTmp]), type='add')
                            self._log(
                                INFO,
                                "[6top] fired timer on mote for reservation: {0}.",
                                (copy.deepcopy(self.ReSF.txBlockedTuples[moteTmp]),),
                            )
                            del self.ReSF.txBlockedTuples[moteTmp]

                    if not txFound: # then maybe it was a ReSF delete
                        for moteTmp in self.ReSF.txDeleteTuples.keys():
                            if moteTmp.id == n:
                                # here the ReSF reservation should first be re added
                                self.ReSF._resf_delayReservation(copy.deepcopy(self.ReSF.txDeleteTuples[moteTmp]), type='delete', cause='badlink')
                                del self.ReSF.txDeleteTuples[moteTmp]

                    # try to execute a delayed ReSF request
                    self.ReSF._resf_scheduleDelayedRequest()

        if not found:  # if we did not find it, assert
            assert False

    def _sixtop_cell_reservation_request(self, neighbor, numCells, dir, timeout, resf=None, backupDuration=None):
        ...
        with self.dataLock:
            if self.settings.sixtopMessaging:
                if neighbor.id not in self.sixtopStates or \
                    ...
                    if resf is None and self.eLLSF is None:  # do normal 6top reservation
                        ...
                        self._sixtop_enqueue_ADD_REQUEST(neighbor, cellList, numCells, dir,
                                                         self.sixtopStates[neighbor.id]['tx']['seqNum'], backupDuration=backupDuration)
                    elif resf is None and self.eLLSF is not None:
                        self._log(
                            DEBUG,
                        ...
                        if cellList != []:  # only send the request if we found (an) appropriate cell(s)
                            self._sixtop_enqueue_ADD_REQUEST(neighbor, cellList, numCells, dir,
                                                             self.sixtopStates[neighbor.id]['tx']['seqNum'], backupDuration=backupDuration)
                    else:  # do ReSF reservation
                        # numCells can be 0
                        # cellList can be []
                        self._sixtop_enqueue_ADD_REQUEST(neighbor, [], 0, dir,
                                                         self.sixtopStates[neighbor.id]['tx']['seqNum'], resf)
                else:
                    # print neighbor.id
                    # print self.id
                    # print neighbor.id
                    # print self.sixtopStates[neighbor.id]
                    self._log(
                        DEBUG,
                    ...
                    if resf is not None:
                        self._log(
                            DEBUG,
                            "[6top] delayed ReSF request because timer still did not fire on mote {0}.",
                            (self.id,),
                        )
                        self.ReSF._resf_delayReservation(resf, type='add')
    ...
    def _sixtop_enqueue_ADD_REQUEST(self, neighbor, cellList, numCells, dir, seq, resf=None, backupDuration=None):
        ...
        newPacket = {
            ...
            'payload': [cellList, numCells, dir, seq, self.engine.getAsn(), resf, backupDuration],
            'retriesLeft': TSCH_MAXTXRETRIES,
            'srcIp': self,
        ...
        if not isEnqueued:
            self._log(
                INFO,
                '[6top] ADD Request did not get enqueued: {0}',
                (resf,),
            )
            # update mote stats
            self._radio_drop_packet(newPacket, 'droppedFailedEnqueue')

            if resf is not None:  # if it did not get enqueued, reschedule it
                self.ReSF._resf_delayReservation(copy.deepcopy(resf), type='add')
                self.ReSF._resf_scheduleDelayedRequest(1000)
        else:
            self._log(
                INFO,
                '[6top] ADD Request got enqueued: {0}',
                (resf,),
            )
            ...
            if resf is not None:
                self.ReSF.txBlockedTuples[neighbor] = copy.deepcopy(resf)
    ...
    def _sixtop_receive_ADD_REQUEST(self, type, smac, payload):
        with self.dataLock:
            ...
            # if there is resf content, it is stored here at index 5 in a 6top REQUEST
            resf = copy.deepcopy(payload[5])

            backupDuration = None
            if self.ReSF and resf is None and payload[6] is not None: # resf should be None!
                backupDuration = payload[6]
            ...
            if smac.id in self.sixtopStates and 'rx' in self.sixtopStates[smac.id] and self.sixtopStates[smac.id]['rx'][
                ...
                self._sixtop_enqueue_RESPONSE(neighbor, [], returnCode, dirNeighbor, seq, resf=resf)

                if self.ReSF is not None and neighbor in self.ReSF.rxBlockedTuples:
                    del self.ReSF.rxBlockedTuples[neighbor]
                return
            ...
            if resf is None and self.eLLSF is None:
                # get blocked cells from other 6top operations
                blockedCells = []
                for n in self.sixtopStates.keys():
                    if n != neighbor.id:
                ...
                # enqueue response
                self._sixtop_enqueue_RESPONSE(neighbor, newCellList, returnCode, newDir, seq, backupDuration=backupDuration)
            else:
                # prepare ReSF payload for response and block tuples
                resfPayload = self.ReSF._resf_receive_ADD_REQUEST(neighbor, resf)

                # TODO for now always return success
                if False:
                    returnCode = IANA_6TOP_RC_NORES  # not enough resources
                else:
                    returnCode = IANA_6TOP_RC_SUCCESS  # enough resources
                ...
                # enqueue response
                self._sixtop_enqueue_RESPONSE(neighbor, [], returnCode, newDir, seq, resf=resfPayload)
    ...
    def _sixtop_enqueue_RESPONSE(self, neighbor, cellList, returnCode, dir, seq, resf=None, backupDuration=None):
        ...
        newPacket = {
            ...
            'code': returnCode,
            'payload': [cellList, len(cellList), dir, seq, resf, backupDuration],
            'retriesLeft': TSCH_MAXTXRETRIES,
            'srcIp': self,
            'dstIp': neighbor,  # currently upstream
            'sourceRoute': [],
    ...
    def _sixtop_receive_RESPONSE(self, type, code, smac, payload):
        ...
        with self.dataLock:
            if self.sixtopStates[smac.id]['tx']['state'] == SIX_STATE_WAIT_ADDRESPONSE:
                ...
                resf = copy.deepcopy(payload[4])
                backupDuration = None
                if self.ReSF is not None and resf is None and payload[5] is not None:
                    backupDuration = payload[5]
                ...
                if seq != self.sixtopStates[neighbor.id]['tx']['seqNum']:
                    ...
                    self.sixtopStates[neighbor.id]['tx']['state'] = SIX_STATE_IDLE
                    self.sixtopStates[neighbor.id]['tx']['blockedCells'] = []
                    # do not check on resf payload, b/c it could be that we are dealing with a corrupt normal 6P response
                    # therefore check on if neighbor is in txBlockedTuples
                    if self.ReSF is not None and neighbor in self.ReSF.txBlockedTuples:
                        del self.ReSF.txBlockedTuples[neighbor]
                    return False
                ...
                if self.sixtopStates[neighbor.id]['tx']['timer']['asn'] == self.engine.getAsn():
                    ...
                    self.sixtopStates[neighbor.id]['tx']['state'] = SIX_STATE_IDLE
                    self.sixtopStates[neighbor.id]['tx']['blockedCells'] = []
                    # do not check on resf payload, b/c it could be that we are dealing with a corrupt normla 6P response
                    # therefore check on if neighbor is in txBlockedTuples
                    # if resf is not None:
                    #     # delay the reservation
                    #     self.ReSF._resf_delayReservation(copy.deepcopy(self.ReSF.txBlockedTuples[neighbor]), type='add')
                    if self.ReSF is not None and neighbor in self.ReSF.txBlockedTuples:
                        del self.ReSF.txBlockedTuples[neighbor]
                    return False
                ...
                if code == IANA_6TOP_RC_SUCCESS:
                    if resf is not None:
                        resfResult = self.ReSF._resf_receive_RESPONSE(neighbor, receivedDir, resf)
                        if not resfResult: # if this is negative, the packet is ignored because the request is not relevant anymore due to an earlier delete on this node
                            self.sixtopStates[neighbor.id]['tx']['state'] = SIX_STATE_IDLE
                            self.sixtopStates[neighbor.id]['tx']['blockedCells'] = []
                        ...
                        # if resf is None:  # only delete when you have received a response for a non-ReSF request, because only none-ReSF response mean there is already a new reservation completed to the new parent
                        #     self._log(
                        #         INFO,
                        #         '[6top] Retransmission event is being removed...',
                        #         (neighbor.id, self.id),
                        #     )
                        #     self.engine.removeEvent((self.id,
                        #                              '_msf_action_parent_change_retransmission'))  # remove the pending retransmission event for MSF
                        ...
                        for (ts, ch, cellDir) in receivedCellList:
                            ...
                            cellList += [(ts, ch, newDir)]
                        self._tsch_addCells(neighbor, cellList, backupDuration=backupDuration)

                        # test for convergence, only in the no-ReSF case
                        if (self.ReSF is None or (self.ReSF is not None and self.settings.minCellsMSF > 0)) \
                                and self.settings.convergeFirst \
                                and not self.isConverged \
                                and len(receivedCellList) > 0 \
                                and neighbor == self.preferredParent \
                                and (newDir == DIR_TX or newDir == DIR_TXRX_SHARED):
                            self.isConverged = True
                        ...
                        # do no count this cell as it will dissappear quite quickly
                        if backupDuration is None or backupDuration == 0:
                            # update counters
                            if newDir == DIR_TX:
                                if neighbor not in self.numCellsToNeighbors:
                                    self.numCellsToNeighbors[neighbor] = 0
                ...
                # only when devices are not powerfull enough. Not used in the simulator
                    ...
                    if resf is not None:
                        # delay the reservation
                        self.ReSF._resf_delayReservation(copy.deepcopy(self.ReSF.txBlockedTuples[neighbor]), type='add')
                        # print self.ReSF.txBlockedTuples[neighbor]
                        # assert False
                        del self.ReSF.txBlockedTuples[neighbor]
                    return True
                    # TODO: increase stats of RC_BUSY
                ...
                if seq != self.sixtopStates[neighbor.id]['tx']['seqNum']:
                    ...
                    self.sixtopStates[neighbor.id]['tx']['state'] = SIX_STATE_IDLE
                    self.sixtopStates[neighbor.id]['tx']['blockedCells'] = []
                    if self.ReSF is not None and neighbor in self.ReSF.txDeleteTuples:
                        del self.ReSF.txDeleteTuples[neighbor]
                    return False
                ...
                if code == IANA_6TOP_RC_SUCCESS:
                    ...
                    if resf is not None:
                        self.ReSF._resf_receive_DELETE_RESPONSE(neighbor, receivedDir, resf)
                    else:
                        # set direction of cells
                        if receivedDir == DIR_TX:
                            newDir = DIR_RX
                ...
                # only when devices are not powerfull enough. Not used in the simulator
                    ...
                    if resf is not None:
                        assert False
                        # self.ReSF._resf_delayReservation(copy.deepcopy(self.ReSF.txBlockedTuples[neighbor]), type='delete')
                        del self.ReSF.txDeleteTuples[neighbor]
    ...
    def _sixtop_receive_RESPONSE_ACK(self, packet):
        with self.dataLock:
            ...
            if self.sixtopStates[packet['dstIp'].id]['rx']['state'] == SIX_STATE_WAIT_ADD_RESPONSE_SENDDONE:
                ...
                code = packet['code']
                resf = packet['payload'][4]

                backupDuration = None
                if self.ReSF is not None and resf is None and packet['payload'][5] is not None:
                    backupDuration = packet['payload'][5]
                ...
                if code == IANA_6TOP_RC_SUCCESS:
                    if resf is not None:
                        self.ReSF._resf_receive_RESPONSE_ACK(neighbor, receivedDir, resf)
                    else:
                        ...
                        self._tsch_addCells(neighbor, confirmedCellList, backupDuration=backupDuration)

                        if backupDuration is not None:
                            self.ReSF.haveAllocatedCells += len(confirmedCellList)
                ...
                # following does not need to happen anymore, happens in ReSF.py
                # if resf is not None:
                #     del self.ReSF.rxBlockedTuples[neighbor]
                ...
                if code == IANA_6TOP_RC_SUCCESS:
                    if resf is not None:
                        self.ReSF._resf_receive_DELETE_RESPONSE_ACK(neighbor, receivedDir, resf)
                    else:
    ...
    def _sixtop_cell_deletion_sender(self, neighbor, tsList, dir, timeout, resf=None):
        with self.dataLock:
            if self.settings.sixtopMessaging:
                if neighbor.id not in self.sixtopStates or (
                    ...
                    if resf is not None:
                        self._log(
                            DEBUG,
                            "[6top] ReSF contents of future DELETE request: {0}",
                            (resf,),
                        )

                    self._sixtop_enqueue_DELETE_REQUEST(neighbor, tsList, len(tsList), dir,
                                                        self.sixtopStates[neighbor.id]['tx']['seqNum'], resf=resf)
                else:
                    self._log(
                        DEBUG,
                    ...
                    if resf is not None:
                        # delay it and be sure to resend it.
                        self.ReSF._resf_delayReservation(copy.deepcopy(resf), type='delete')
                        self._log(
                            DEBUG,
                            "[6top] delayed ReSF delete request on mote {0}.",
                            (self.id,),
                        )
            else:
                # log
    ...
    def _sixtop_enqueue_DELETE_REQUEST(self, neighbor, cellList, numCells, dir, seq, resf=None):
        ...
        newPacket = {
            ...
            'payload': [cellList, numCells, dir, seq, self.engine.getAsn(), resf],
            'retriesLeft': TSCH_MAXTXRETRIES,
            'srcIp': self,
            'dstIp': neighbor,  # currently upstream
            'sourceRoute': [],
        ...
        if not isEnqueued:
            ...
            self.sixtopStates[neighbor.id]['tx']['state'] = SIX_STATE_SENDING_REQUEST
            if resf is not None:
                self.ReSF.txDeleteTuples[neighbor] = copy.deepcopy(resf)
    ...
    def _sixtop_receive_DELETE_REQUEST(self, type, smac, payload):
        ...
        with self.dataLock:
            ...
            if resf is not None and len(cellList) == 0:
                pass
            elif resf is not None and len(cellList) > 0:
                assert False
            else:
            ...
            # enqueue response
            self._sixtop_enqueue_RESPONSE(neighbor, cellList, returnCode, newDir, seq, resf)
    ...
    def _tsch_enqueue(self, packet):
        ...
        elif not (self.getTxCells() or self.getSharedCells() or (self.ReSF is not None and len(self.ReSF.txTuples[self.preferredParent]) != 0)):
    ...
    def _tsch_action_activeCell(self):
        ...
        with self.dataLock:
            ...
            # do not trigger it when the cell has a ReSF backup duration
            if self._msf_is_enabled() and not cell['resf'] and cell['backupDuration'] is None:
            ...
            if cell['dir'] == DIR_RX:
                ...
                if self.ReSF is not None \
                        and self.ReSF.giveReSFPriority \
                        and self.settings.resfPriority == 1 \
                        and not cell['resf']:
                    pass # do nothing if it is another cell and you have to give priority
                    self._log(
                        INFO,
                        "[tsch] Passing BACKUP cell {0} because we have to give priority to ReSF cells.",
                        (ts,),
                    )
                else:
                    ...
                    if self.settings.convergeFirst and self.engine.asn >= self.engine.asnInitExperiment and self.engine.asn <= self.engine.asnEndExperiment:
                        self.resfCellsTotal += 1
                        if self.pktToSend and cell['resf']:
                            self.resfCellsTx += 1
                        elif cell['resf']:
                            self.resfCellsNoTx += 1
                    elif not self.settings.convergeFirst:
                        self.resfCellsTotal += 1
                        if self.pktToSend and cell['resf']:
                            self.resfCellsTx += 1
                    ...
                    if self.pktToSend:
                        ...
                        if self._msf_is_enabled() and not cell['resf']:
                            self._msf_signal_cell_used(cell['neighbor'], cell['dir'], DIR_TX, pkt['type'])

                        # do not increment if it is not a resf cell
                        if not cell['resf']:
                            cell['numTx'] += 1
                    ...
                    if self.txQueue and self.backoffBroadcast == 0:
                        for pkt in self.txQueue:
                            ...
                            if pkt['type'] == APP_TYPE_JOIN and len(
                                    self.getTxCells(neighbor=pkt['nextHop'][0], includeReSF=True)) + len(
                ...
                # if self.pktToSend is not None and self.pktToSend['type'] == RPL_TYPE_DIO:
                #     self._log(
                #         INFO,
                #         "[DIO] send DIO",
                #     )
                ...
                if self.pktToSend:
                    ...
                    if pkt['type'] == IANA_6TOP_TYPE_RESPONSE:
                        # if 'state' not in self.sixtopStates[self.pktToSend['nextHop'][0].id]['rx']:
                        #     print self.id
                        #     print self.pktToSend['nextHop'][0].id
                        #     print self.sixtopStates[self.pktToSend['nextHop'][0].id]['rx']
                        #     print self.sixtopStates
    ...
    def _tsch_addCells(self, neighbor, cellList, resf=False, resfUniqueId=None, backupDuration=None):
        """ adds cell(s) to the schedule """

        with self.dataLock:
            for cell in cellList:
                # in case of adding resf cells, do not overwrite an already existing cell and give preference to eLLSF cells
                # in the second case, only do not add it if it is the restricted mode of eLLSF
                if (resf and cell[0] in self.schedule.keys()) or (resf and self.settings.ellsfMode == 'restricted' and cell[0] in self.eLLSF.ELLSF_TIMESLOTS):
                    # if (resf and cell[0] in self.schedule.keys()):
                    #     self._log(
                    #         INFO,
                    #         "[tsch] did not add cell ts={0} ch={1} dir={2} with {3} because of schedule collision",
                    #         (cell[0], cell[1], cell[2],
                    #          neighbor.id if not type(neighbor) == list else self.BROADCAST_ADDRESS),
                    #     )
                    continue  # we give preference to other cells, so do not add the resf cell this time
                elif (self.settings.ellsfMode == 'all' or self.settings.ellsfMode == 'halfrestricted') and not resf and \
                        cell[0] in self.schedule.keys() and self.schedule[cell[0]]['resf']:
                    # in the case resf cells were added first and afterwards in the same slotframe a ellsf cell is added
                    del self.schedule[cell[0]]
                    # if self.settings.convergeFirst and self.engine.asn >= self.engine.asnInitExperiment and self.engine.asn <= self.engine.asnEndExperiment:
                    #     self._log(
                    #         INFO,
                    #         "[tsch] deleting cell at ts={0} ch={1} dir={2} with {3} because of eLLSF cell",
                ...
                # [debug] shows schedule collision that can be interfered with minRssi or larger level
                    ...
                    'resf': resf,
                    'resf_unique_id': resfUniqueId,
                    'backupDuration': backupDuration,
                }
                # log
                # self._log(
                #     INFO,
                #     "[tsch] add cell (resf: {5}) ts={0} ch={1} dir={2} with {3} (and backupDuration {4})",
                #     (cell[0], cell[1], cell[2], neighbor.id if not type(neighbor) == list else BROADCAST_ADDRESS, str(backupDuration), str(resf)),
                # )
            self._tsch_schedule_activeCell()
    ...
    def _tsch_removeCells(self, neighbor, tsList):
        ...
        with self.dataLock:
            for cell in tsList:
                ...
                if self.ReSF is not None:
                    assert not self.schedule[cell]['resf']
    ...
    def _tsch_add_minimal_cell(self):
        ...
        for c in range(0, self.settings.nrMinimalCells):
            self._tsch_addCells(self._myNeighbors(), [(c, c, DIR_TXRX_SHARED)])
        ...
        with self.dataLock:
            ...
            if isACKed:
                ...
                if self.settings.convergeFirst and self.engine.asn >= self.engine.asnInitExperiment and self.engine.asn <= self.engine.asnEndExperiment:
                    ...
                    #     assert False
                    # if self.id == 0:
                    #     print self.pktToSend['type']
                    #     assert False

                    # if self.id == 1 and self.pktToSend['type'] == APP_TYPE_DATA:
                    #     self.datapkts += 1
                    #     print 'data'
                    #     print self.datapkts
                    #     print self.requestspkts
                    #     print self.responsespkts
                    # elif self.id == 1 and self.pktToSend['type'] == IANA_6TOP_TYPE_REQUEST:
                    #     self.requestspkts += 1
                    #     print 'requests'
                    #     print self.requestspkts
                    # elif self.id == 1 and self.pktToSend['type'] == IANA_6TOP_TYPE_RESPONSE:
                    #     self.responsespkts += 1
                    #     print 'responses'
                    #     print self.responsespkts
                    # elif self.id == 1:
                    #     self.othertypes.append(self.pktToSend['type'])
                    #     print self.othertypes
                elif not self.settings.convergeFirst:
                    self.nrTxDataRxAck += 1

                # update schedule stats
                if not self.schedule[ts]['resf']:
                ...
                if self.pktToSend[
                    'type'] == IANA_6TOP_TYPE_RESPONSE:  # received an ACK for the response, handle the schedule
                ...
                if self.pktToSend['type'] == APP_TYPE_DATA:  #
                    self._log(
                        DEBUG,
                        "[tsch] UNsuccessfully sent DATA packet (NACK)",
                    )
                ...
                if self.txQueue[i]['retriesLeft'] == 0:
                    if len(self.txQueue) == TSCH_QUEUE_SIZE:
                        ...
                        if self.pktToSend['type'] == APP_TYPE_DATA:
                            if self.settings.convergeFirst and self.pktToSend['payload'][
                                1] >= self.engine.asnInitExperiment and self.engine.asn <= self.engine.asnEndExperiment:
                                self.pktDropMac += 1
                            elif not self.settings.convergeFirst:
                                self.pktDropMac += 1
                        ...
                        if self.pktToSend['type'] == IANA_6TOP_TYPE_REQUEST:
                            ...
                            if self.ReSF is not None and len(self.pktToSend['payload']) > 5 and \
                                    self.pktToSend['payload'][5] is not None:  # index 5 for a request
                                if self.pktToSend['dstIp'] in self.ReSF.txBlockedTuples:
                                    del self.ReSF.txBlockedTuples[self.pktToSend['dstIp']]
                                    self.ReSF._resf_delayReservation(copy.deepcopy(self.pktToSend['payload'][5]),
                                                                     type='add')
                                elif self.pktToSend['dstIp'] in self.ReSF.txDeleteTuples:
                                    del self.ReSF.txDeleteTuples[self.pktToSend['dstIp']]
                                    self.ReSF._resf_delayReservation(copy.deepcopy(self.pktToSend['payload'][5]),
                                                                     type='delete', cause='badlink')
                                    self._log(
                                        DEBUG,
                                        "[6top] possibly delaying a ReSF DELETE on mote {0}, because of NACK (queue maxed out).",
                                        (self.id, ),
                                    )
                                else:
                                    assert False
                            ...
                            if self.ReSF is not None and len(self.pktToSend['payload']) > 4 and \
                                    self.pktToSend['payload'][4] is not None:  # index 4 for a request
                                if self.pktToSend['dstIp'] in self.ReSF.rxBlockedTuples: # check this b/c it is possible it is a delete response
                                    del self.ReSF.rxBlockedTuples[self.pktToSend['dstIp']]
                    else:
                        if self.pktToSend['type'] != APP_TYPE_DATA:
                            # update mote stats
                            self._stats_incrementMoteStats('droppedMacRetries')
                            ...
                            if self.pktToSend['type'] == IANA_6TOP_TYPE_REQUEST:
                                ...
                                if self.ReSF is not None and len(self.pktToSend['payload']) > 5 and \
                                    ...
                                    if self.pktToSend['dstIp'] in self.ReSF.txBlockedTuples:
                                        ...
                                        self._log(
                                            DEBUG,
                                            "[6top] possibly delaying a ReSF DELETE on mote {0}, because of NACK.",
                                            (self.id,),
                                        )
                                    else:
                                        assert False
                ...
                # reset backoff in case of shared slot or in case of a tx slot when the queue is empty
                if self.schedule[ts]['dir'] == DIR_TXRX_SHARED or (
                        self.schedule[ts]['dir'] == DIR_TX and not self.txQueue):
                    if self.schedule[ts]['dir'] == DIR_TXRX_SHARED and not self._isBroadcast(self.schedule[ts][
                        'neighbor']):
                ...
                if self.settings.convergeFirst and self.engine.asn >= self.engine.asnInitExperiment and self.engine.asn <= self.engine.asnEndExperiment:
                    self.nrTxData += 1
                    # if self.id == 1:
                    #     print 'sending a txdata from node 1'
                    #     print self.pktToSend['type']
                    # self.broadcastpackets.append(self.pktToSend['type'])
                    # print self.id
                    # print self.broadcastpackets
                elif not self.settings.convergeFirst:
                    self.nrTxData += 1
                self.txQueue.remove(self.pktToSend)
                self._tsch_resetBroadcastBackoff()
                ...
                if self.pktToSend[ 'type'] == APP_TYPE_DATA:  #
                    self._log(
                        DEBUG,
                        "[tsch] UNsuccessfully sent DATA packet (NO RESPONSE)",
                    )
                        ...
                        if self.pktToSend['type'] == IANA_6TOP_TYPE_REQUEST:
                            ...
                            if self.ReSF is not None and len(self.pktToSend['payload']) > 5 and \
                                ...
                                if self.pktToSend['dstIp'] in self.ReSF.txBlockedTuples:
                                    ...
                                    self._log(
                                        DEBUG,
                                        "[6top] possibly delaying a ReSF DELETE on mote {0}, because of no ACK (queue maxed out).",
                                        (self.id,),
                                    )
                                else:
                                    assert False
                            ...
                            if self.ReSF is not None and len(self.pktToSend['payload']) > 4 and \
                                    self.pktToSend['payload'][4] is not None:  # index 4 for a request
                                if self.pktToSend[
                                    'dstIp'] in self.ReSF.rxBlockedTuples:  # check this b/c it is possible it is a delete response
                                    del self.ReSF.rxBlockedTuples[self.pktToSend['dstIp']]
                    else:
                        if self.pktToSend['type'] != APP_TYPE_DATA:
                            ...
                            if self.pktToSend['type'] == IANA_6TOP_TYPE_REQUEST:
                                ...
                                if self.ReSF is not None and len(self.pktToSend['payload']) > 5 and \
                                    ...
                                    if self.pktToSend['dstIp'] in self.ReSF.txBlockedTuples:
                                        ...
                                        self._log(
                                            DEBUG,
                                            "[6top] possibly delaying a ReSF DELETE on mote {0}, because of no ACK.",
                                            (self.id,),
                                        )
                                    else:
                                        assert False
    ...
    def radio_rxDone(self, type=None, code=None, smac=None, dmac=None, srcIp=None, dstIp=None, srcRoute=None,
        ...
        with self.dataLock:
            ...
            if smac and self in dmac:  # layer 2 addressing
                ...
                if self._msf_is_enabled() and self.isSync and not self.schedule[ts]['resf']:
                    self._msf_signal_cell_used(self.schedule[ts]['neighbor'], self.schedule[ts]['dir'], DIR_RX, type)

                # if self.ReSF is not None and self.id == 2 and type == APP_TYPE_DATA and srcIp.id == 3 and smac.id == 3:
                #     self._log(
                #         INFO,
                #         "[tsch] Received from 3 at 2 originating from 3 (uniqueId == {0})",
                #         (payload[3],)
                #     )

                if self.ReSF is not None and (self.settings.resfChangeParent == 1 and (self.settings.resfChangeParentPolicy == 'keepalive' or self.settings.resfChangeParentPolicy == 'combined')) and type == APP_TYPE_DATA:
                    self.ReSF._resf_reservation_used(srcIp.id, smac.id, payload[3])

                if dstIp != BROADCAST_ADDRESS:  # unicast packet
                    self._logChargeConsumed(CHARGE_RxDataTxAck_uC)
                    if self.settings.convergeFirst and self.engine.asn >= self.engine.asnInitExperiment and self.engine.asn <= self.engine.asnEndExperiment:
                        self.nrRxDataTxAck += 1
                        # print 'come here in unicast'
                        # print type
                    elif not self.settings.convergeFirst:
                        self.nrRxDataTxAck += 1
                else:  # broadcast
                ...
                if dstIp == BROADCAST_ADDRESS:
                    if type == RPL_TYPE_DIO:
                        ...
                        # if self.id == 1 and self.engine.asn >= self.engine.asnInitExperiment:
                        #     assert False
    ...
    def _init_stack(self):
        ...
        if self.ReSF and self.settings.resfNoMSF == 'no':
            self.ReSF._resf_schedule_housekeeping()
    ...
    def getTxCells(self, neighbor=None, includeReSF=False):
        with self.dataLock:
            if neighbor is None:
                if includeReSF:
                ...
                if includeReSF:
                    ...
                    return [(ts, c['ch'], c['neighbor']) for (ts, c) in self.schedule.items() if
                            c['dir'] == DIR_TX and c['neighbor'] == neighbor and not c['resf']]

    def getRxCells(self, neighbor=None, includeReSF=False):
        with self.dataLock:
            if neighbor is None:
                if includeReSF:
                ...
                if includeReSF:
                    ...
                    return [(ts, c['ch'], c['neighbor']) for (ts, c) in self.schedule.items() if
                            c['dir'] == DIR_RX and c['neighbor'] == neighbor and not c['resf']]

    def getSharedCells(self, neighbor=None):
        with self.dataLock:
            if neighbor is None:
                return [(ts, c['ch'], c['neighbor']) for (ts, c) in self.schedule.items() if
                        c['dir'] == DIR_TXRX_SHARED]
            else:
                return [(ts, c['ch'], c['neighbor']) for (ts, c) in self.schedule.items() if
        ...
        with self.dataLock:
            ...
            for p in self.txQueue:
                if not self.settings.convergeFirst and p['type'] == APP_TYPE_DATA:
                    dataPktQueues += 1
                elif self.settings.convergeFirst and p['type'] == APP_TYPE_DATA and p['payload'][
                    1] >= self.engine.asnInitExperiment and p['payload'][1] <= self.engine.asnEndExperiment:
                    dataPktQueues += 1

            returnVal = copy.deepcopy(self.motestats)
            returnVal['numTxCells'] = len(self.getTxCells(includeReSF=False))
            returnVal['numTxCellsReSF'] = len(self.getTxCells(includeReSF=True)) - len(
                self.getTxCells(includeReSF=False))
            returnVal['numRxCells'] = len(self.getRxCells(includeReSF=False))
            returnVal['numRxCellsReSF'] = len(self.getRxCells(includeReSF=True)) - len(
                self.getRxCells(includeReSF=False))
            returnVal['numDedicatedCells'] = len(
                [(ts, c) for (ts, c) in self.schedule.items() if type(self) == type(c['neighbor']) and not c['resf']])
            ...
            returnVal['numTx'] = sum(
                [cell['numTx'] for (_, cell) in self.schedule.items() if not cell['resf']])  # do not count resf cells
